# 📊 AdventureWorks Sales & Customer Analytics (Power BI)

An **end-to-end business intelligence project** built using **Power BI** on the AdventureWorks dataset.  
This project demonstrates my ability to **model data, write advanced DAX**, and **convert raw data into actionable business insights** — not just create dashboards.

---

## 🔍 Project Objective

The goal of this project is to **analyze sales performance, customer behavior, product profitability, and regional trends**, while answering real business questions such as:

- How is revenue and profit trending over time?  
- Who are the most valuable customers?  
- Which products drive revenue vs returns?  
- How close are we to monthly targets?  
- What happens to profit if prices change?  
- Which regions perform best?  

This project is designed specifically for **data analyst / BI analyst / Power BI developer interviews**.

---

## 🛠 Tools & Technologies

- **Power BI Desktop**  
- **DAX (Data Analysis Expressions)**  
- **Power Query (ETL)**  
- **Data Modeling**  
- **Interactive Dashboards**  
- **What-If Parameters**  

---

## 📈 Key KPIs Tracked

- **Total Orders:** 25.2K  
- **Total Revenue:** ₹24.9M  
- **Total Profit:** ₹10.5M  
- **Return Rate:** 2.2%  
- **Distinct Customers:** 17.4K  
- **Revenue per Customer:** ₹1.4K  

---

## 🧩 Dashboard Pages Overview

### 1️⃣ Executive Sales Overview
- Revenue, Orders, Profit & Return Rate KPIs  
- Revenue trend with forecasting  
- Order distribution by product category  
- Monthly comparison with previous month  
- Most ordered product type  
- Product with highest return rate  

**Business Insight:** Revenue shows strong growth post-2021, driven mainly by **Accessories and Bikes**.

---

### 2️⃣ Customer Analytics
- Customer trend analysis over time  
- Revenue per customer tracking  
- Customer segmentation by:
  - Income group  
  - Occupation  
- Top 100 customers by revenue  
- Best customer spotlight (dynamic)  

**Business Insight:** A small group of customers contributes disproportionately to total revenue → opportunity for **loyalty programs**.

---

### 3️⃣ Product Performance & What-If Analysis
- Monthly Orders vs Target  
- Monthly Revenue vs Target  
- Monthly Profit vs Target  
- What-If price adjustment slider  
- Comparison of actual vs adjusted profit  
- Product-level metric switching:
  - Orders  
  - Revenue  
  - Profit  
  - Returns  
  - Return %  

**Business Insight:** Even small price adjustments significantly impact profitability for high-volume products like **Mountain Tire Tube**.

---

### 4️⃣ Geographic Analysis
- Interactive world map  
- Region-wise filtering:
  - Europe  
  - North America  
  - Pacific  
- Country-level performance comparison  

**Business Insight:** North America and Europe dominate revenue, while Pacific shows growth potential.

---

## 🧠 Advanced Features Used

- ✔ Star schema data modeling  
- ✔ Time intelligence DAX  
- ✔ Dynamic titles & KPIs  
- ✔ What-If parameter (Price Adjustment %)  
- ✔ Target vs Actual comparison  
- ✔ Drill-through & slicers  
- ✔ Conditional formatting  

---

## 📌 Sample DAX Concepts Implemented

- Revenue, Profit, Return Rate measures  
- Customer ranking (Top N)  
- Dynamic metric switching  
- Week-based time analysis  
- Price sensitivity simulation  

---

## 🎯 Who This Project Is For

- **Recruiters** evaluating Power BI / Data Analyst skills  
- **Hiring managers** looking for business-focused dashboards  
- Anyone wanting to see **real-world BI problem solving**  

---

## 📂 Dataset

- **AdventureWorks** (Microsoft sample dataset)

---

## 📬 Contact

**Deepanshu Kashyap**  
📧 Email: deepanshukashyap531@gmail.com  
🔗 LinkedIn: [https://www.linkedin.com/in/deepanshu577](https://www.linkedin.com/in/deepanshu577)  

---

## ⭐ Final Note

This project focuses on **decision-making, not decoration**.  
Every visual answers a **real business question**.

If you like this project, feel free to ⭐ the repository.  
